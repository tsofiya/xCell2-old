...
working.dir = getwd()
source(paste0(working.dir,'/R/xCell.R'))
source(paste0(working.dir,'/Dev_scripts/xCell_dev_functions.R'))

# -- read reference data sets -- #
data_sets = c('blueprint')
data_types = c('seq')

families=read.table(file.path(working.dir,'/Dev_scripts/cells_families.txt'),sep="\t",header=TRUE,row.names=1, as.is=TRUE)
dependencies = read.types.dependencies(working.dir,'/Dev_scripts/types_dependecies.txt')

#read DB: to change to user data
ref <- BlueprintEncodeData()

# --- create train and test sets --- #
#create.train.test.ref.sets(working.dir,data_sets,nsamples=4)#TODO no need for train test
#add here ref= celldex...
...
# Hard coded!! check if thats what it should do..
ref$type<- rep(data_types[1], ncol(ref))
...
# not overexpressed - remove cancer genes - TODO: change this later, remind Dvir to send file
#not_over_expressed = unlist(read.delim(file.path(working.dir,'not_overexpressed_genes.txt'),sep='\n'))

create.signatures(ref,dependencies,not_over_expressed,rownames(families)[families[,3]=="Epithelial"],working.dir)
